...
async function handleLoginSubmit(e) {
  ...
  try {
    ...
    if (!isAuthenticated) {
      if ((userField === 'احمد مجدي' || userField === 'admin') && passField === 'admin123') {
...
function renderInventoryTable(filterTerm = '') {
  ...
  tbody.innerHTML = filtered.length === 0
    ? '<tr><td colspan="7" class="py-8 text-center text-slate-500">لا توجد قطع غيار مطابقة للبحث.</td></tr>'
    : filtered.map(v => {
        const prod = dbData.products.find(p => p.id === v.product_id) || {};
    ...
    : filtered.map(s => {
        ...
        return `
          <tr class="hover:bg-slate-900/40">
            ...
            <td class="py-4 px-6 text-center">
              <button onclick="viewInvoiceReceipt('${s.id}')" class="bg-blue-600/10 hover:bg-blue-600/20 text-blue-500 hover:text-blue-400 border border-blue-500/20 px-3 py-1.5 rounded-lg text-xs font-bold transition-all">
                عرض التفاصيل
              </button>
            </td>
          </tr>`;
